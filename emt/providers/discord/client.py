...
class DiscordClient:
    ...
    def get_member_by_id(self, user_id) -> dict:
        response = requests.get(
            f"{self.api}/guilds/{self.server}/members/{user_id}", headers=self.headers
        )
    ...
    def create_invite(self, channel_id: str = None, max_uses: int = 1) -> dict:
        ...
        response = requests.post(
            f"{self.api}/channels/{channel_id}/invites",
            headers=self.headers,
            json={"unique": True, "max_age": 604800, "max_uses": max_uses},
        )

        return response.json()

    def delete_invite(self, code: str) -> bool:
        response = requests.delete(f"{self.api}/invites/{code}", headers=self.headers)

        return response.status_code == 204

    def get_channels(self, guild_id: str) -> dict:
        response = requests.get(
            f"{self.api}/guilds/{guild_id}/channels", headers=self.headers
        )
    ...
    def common_channel(self) -> dict:
        ...
        return [
            ...
            and channel["type"] == 0
            and channel["name"] == self.guest_channel_name
        ][0]
